...
public class WoodyAgent_menv2 : Agent
{
    //------------------------------------------------------------------------------------------
    ...
    [HideInInspector]
    public WoodyAgent_menv2 otheragent;
    ...
    Transform l1, l2, l3, r1, r2, r3; // the three holding points on the board
    ...
    float force_magnitude = 0.0f;  // the force magnitude adds to each jointf
    [HideInInspector]
    public float terminate_steps = 0f, count_steps = 0f;     // the max steps needs for each episode, use to calculate the time penalty
    // public float count_steps=0f; // to check the current trained steps inorder to terminate the environment
    ...
    public override void Initialize()
    {
        ...
        // woodyarea = GameObject.Find("WoodyArea").GetComponent<WoodyArea_menv2>();
        ...
        Debug.Log("action_count: " + action_count);
    }

    void GetConstraintJoint()  // to make sure the upper arm joint constraint to be a small range
    {
        CharacterJoint upper_arm_left = FindChildObjectwithName(this.transform, "upper_arm_left").GetComponent<CharacterJoint>();
        CharacterJoint upper_arm_right = FindChildObjectwithName(this.transform, "upper_arm_right").GetComponent<CharacterJoint>();

        SoftJointLimit highTwistLimit = upper_arm_left.highTwistLimit;
        highTwistLimit.limit = 10;
        SoftJointLimit swing1Limit = upper_arm_left.swing1Limit;
        swing1Limit.limit = 10f;
        SoftJointLimit swing2Limit = upper_arm_left.swing2Limit;
        swing2Limit.limit = 10f;
        
        upper_arm_left.highTwistLimit = highTwistLimit;
        upper_arm_left.swing1Limit = swing1Limit;
        upper_arm_left.swing2Limit= swing2Limit;

        upper_arm_right.highTwistLimit = highTwistLimit;
        upper_arm_right.swing1Limit = swing1Limit;
        upper_arm_right.swing2Limit = swing2Limit;
    }
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        if (use_support == 0 && count_steps > 40)
        {
    ...
    void CheckBodyPartsDist()
    {
        if(woodyarea.ComputeBodyPartsBreak(this.name))
        {
            if (woodyarea.multi_env == 1)
                {
                    this.OnEpisodeBegin();
                    otheragent.OnEpisodeBegin();
                }
                else
                {
                    Debug.Log( " Episode ends at body parts break !!!!!!!!!!!!! ");
    ...
    void TakeAction7(ActionSegment<float> vectoraction)
    {
        ...
        // Debug.Log( " TakeAction0 " + this.name + ", " + " action is: " + vectoraction[0].ToString("F3") + ", " + vectoraction[1].ToString("F3") + 
    ...
    public override void OnEpisodeBegin()
    {
        ...
        if (use_support == 1)
        {
            if (episodecounter > remove_support_episodes) use_support =0;
        }
        
        // if (episodecounter > remove_support_episodes && remove_support_episodes !=0) use_support =0;
    ...
    void SetResetParameters()
    {
        ...
        observe_index = m_ResetParams.GetWithDefault("observe_index", 7f);
        action_index = m_ResetParams.GetWithDefault("action_index", 7f);
        use_support = m_ResetParams.GetWithDefault("use_support", 0f);
        remove_support_episodes = m_ResetParams.GetWithDefault("remove_support_episodes", 0f);
        ...
        board_dense_reward_index = m_ResetParams.GetWithDefault("board_dense_reward_index", 3f);
        board_sparse_reward_index = m_ResetParams.GetWithDefault("board_sparse_reward_index", 0f);

        ball_balance_dense_reward_index = m_ResetParams.GetWithDefault("ball_balance_dense_reward_index", 2f);
    ...
    void Observation5(VectorSensor sensor) //139 - 16 = 123 + 3 = 126
    {
        sensor.AddObservation(target.position - woodyarea.transform.position); // 3
        
        sensor.AddObservation(ball.position - woodyarea.transform.position);
        sensor.AddObservation(ball.rotation);
        sensor.AddObservation(ballrigid.velocity);
        sensor.AddObservation(ballrigid.angularVelocity); //16

        sensor.AddObservation(ball.position - target.position);
        sensor.AddObservation(Vector3.Distance(ball.position, target.position)); //20
        //
        sensor.AddObservation(board.position - woodyarea.transform.position);
        sensor.AddObservation(board.rotation);
        sensor.AddObservation(boardrigid.velocity);
        sensor.AddObservation(boardrigid.angularVelocity); //33

        sensor.AddObservation(board.position - board_target);
        sensor.AddObservation(Vector3.Distance(board.position, board_target)); //37
        // 
        sensor.AddObservation(Vector3.Distance(l1.position, al1.position));
        // sensor.AddObservation(Vector3.Distance(l2.position, al2.position));
        // sensor.AddObservation(Vector3.Distance(l3.position, al3.position));
        ...
        // sensor.AddObservation(l2.position - al2.position);
        // sensor.AddObservation(l3.position - al3.position);
        sensor.AddObservation(r1.position - ar1.position);    //45
        // sensor.AddObservation(r2.position - ar2.position);
        // sensor.AddObservation(r3.position - ar3.position);
        // 13*6=78
        sensor.AddObservation(upper_arm_left.transform.position - woodyarea.transform.position);
        sensor.AddObservation(upper_arm_left.transform.rotation);
        sensor.AddObservation(upper_arm_left.velocity);
        sensor.AddObservation(upper_arm_left.angularVelocity); //58
        
        sensor.AddObservation(lower_arm_left.transform.position - woodyarea.transform.position);
        sensor.AddObservation(lower_arm_left.transform.rotation);
        sensor.AddObservation(lower_arm_left.velocity); 
        sensor.AddObservation(lower_arm_left.angularVelocity); //71
        
        sensor.AddObservation(hand_left.transform.position - woodyarea.transform.position);
        sensor.AddObservation(hand_left.transform.rotation);
        sensor.AddObservation(hand_left.velocity);
        sensor.AddObservation(hand_left.angularVelocity); //84
        
        sensor.AddObservation(upper_arm_right.transform.position - woodyarea.transform.position);
        sensor.AddObservation(upper_arm_right.transform.rotation);
        sensor.AddObservation(upper_arm_right.velocity); 
        sensor.AddObservation(upper_arm_right.angularVelocity); 
        
        sensor.AddObservation(lower_arm_right.transform.position - woodyarea.transform.position); 
        sensor.AddObservation(lower_arm_right.transform.rotation); 
        sensor.AddObservation(lower_arm_right.velocity); 
        sensor.AddObservation(lower_arm_right.angularVelocity); 
        
        sensor.AddObservation(hand_right.transform.localPosition - woodyarea.transform.position);
        sensor.AddObservation(hand_right.transform.localRotation);
        sensor.AddObservation(hand_right.velocity);
        sensor.AddObservation(hand_right.angularVelocity);
