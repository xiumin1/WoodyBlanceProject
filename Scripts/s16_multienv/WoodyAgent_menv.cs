...
public class WoodyAgent_menv : Agent
{
    //------------------------------------------------------------------------------------------
    ...
    [HideInInspector]
    public WoodyAgent_menv otheragent;
    ...
    [HideInInspector]
    public Transform al1, al2, al3, ar1, ar2, ar3;// the three holding points on the agent hands
    ...
    [HideInInspector]
    public float terminate_steps = 0f, count_steps = 0f;    // the max steps needs for each episode, use to calculate the time penalty
    // public float count_steps = 0f; // to check the current trained steps in order to terminate the environment in .exe directly
    ...
    string logbegin = ""; // to simply hold the onepisodebegin info
    ...
    IKSolver left_iks, right_iks;
    HandReach_ikrl handreach;
    bool currentEpisodeConnected = false;
    // int stepcounts = 0;
    // float time = 0f;
    [HideInInspector]
    public float handreach_reward = 0f, boardreach_reward=0f, ballreach_reward = 0f; // the following three rewards is to adding them into the observation as extra 3 dimensions, to pass them into the python API for CRPO approach
    public override void Initialize()
    {

        m_ResetParams = Academy.Instance.EnvironmentParameters;
        
        // woodyarea = GameObject.Find("WoodyArea").GetComponent<WoodyArea_mtarget>();
        woodyarea = this.GetComponentInParent<WoodyArea_menv>();
        // woodyarea.AreaInit();

        target = woodyarea.target.Find("tmark");
        ...
        if(this.name == "Agent1")
        {
            l1 = woodyarea.a1L.Find("l1");
            l2 = woodyarea.a1L.Find("l2");
            l3 = woodyarea.a1L.Find("l3");

            r1 = woodyarea.a1R.Find("r1");
            r2 = woodyarea.a1R.Find("r2");
            r3 = woodyarea.a1R.Find("r3");
        }

        if(this.name == "Agent2")
        {
            l1 = woodyarea.a2L.Find("l1");
            l2 = woodyarea.a2L.Find("l2");
            l3 = woodyarea.a2L.Find("l3");

            r1 = woodyarea.a2R.Find("r1");
            r2 = woodyarea.a2R.Find("r2");
            r3 = woodyarea.a2R.Find("r3");
        }

        l1.GetComponent<FixedJoint>().connectedBody = boardrigid;
        r1.GetComponent<FixedJoint>().connectedBody = boardrigid;

        // this.transform.GetComponent<Unity.MLAgents.Policies.BehaviorParameters>().BrainParameters.VectorObservationSize = 107;
        ...
        handreach = this.GetComponent<HandReach_ikrl>();
        left_iks = this.transform.FindChildObjectwithName("hl_ik_end").GetComponent<IKSolver>();
        right_iks = this.transform.FindChildObjectwithName("hr_ik_end").GetComponent<IKSolver>();

        // Debug.Log(this.name + "agent init -------------");
    } 
    ...
    {
        // the count_steps speed in observation collection is slower than action decision make, ex. observation collect step is the same as training step, 
        // but during the same training step, there will be 5 action steps, based on the decision_make = 5, so count_steps = training step
        count_steps += 1f;
        // if (count_steps<5)
        //     Debug.Log("collected observations-----" + count_steps);
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        // decision_count += 1;
        // if (count_steps<5f && decision_count==5)
        // {
        //     decision_count = 0;
        //     Vector3 left = new Vector3(vectoraction[0], vectoraction[1], vectoraction[2]);
        //     Vector3 right = new Vector3(vectoraction[3], vectoraction[4], vectoraction[5]);
        //     Debug.Log(" left action is: " + left.ToString("F3") + ", right action is: " + right.ToString("F3"));
        // }
        ...
        if (print_rewardlog == 1f) 
        {
            Debug.Log(this.name +"----" +  log );
            // Debug.Log(woodyarea.name + ", "+this.name+", "+ logbegin + ", " + log);
        }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        logbegin += woodyarea.name + ", " + this.name + " at episodes: " + episodecounter.ToString() + ", total steps: "+ count_steps.ToString();
        // Debug.Log(logbegin);
    ...
    void SetResetParameters()
    {
        ...
        terminate_steps = m_ResetParams.GetWithDefault("terminate_steps", 200f);
        ...
        ball_balance_ratio = m_ResetParams.GetWithDefault("ball_balance_ratio", 0.14f);
        board_ratio = m_ResetParams.GetWithDefault("board_ratio", 0.85f);
        ...
        action_index = m_ResetParams.GetWithDefault("action_index", 2f);
    ...
    void Observation2(VectorSensor sensor) //49
    {
        //3
        sensor.AddObservation(target.position);
        //17
    ...
    void Observation3(VectorSensor sensor) //49
    {
        //3
        sensor.AddObservation(target.position - woodyarea.transform.position); //to get the relative positive to the woodyarea, which means the relative position for each environment
        //17
        sensor.AddObservation(ball.position - woodyarea.transform.position);
        sensor.AddObservation(ball.rotation);
        sensor.AddObservation(ballrigid.velocity);
        sensor.AddObservation(ballrigid.angularVelocity);
        sensor.AddObservation(ball.position - target.position);
        sensor.AddObservation(Vector3.Distance(ball.position, target.position));
        //17
        sensor.AddObservation(board.position - woodyarea.transform.position);
        sensor.AddObservation(board.rotation);
        sensor.AddObservation(boardrigid.velocity);
        sensor.AddObservation(boardrigid.angularVelocity);
        ...
        sensor.AddObservation(Vector3.Distance(board.position, board_target));
        //12
        sensor.AddObservation(l1.position-woodyarea.transform.position);
        sensor.AddObservation(l1.GetComponent<Rigidbody>().velocity);
        sensor.AddObservation(r1.position-woodyarea.transform.position);
        sensor.AddObservation(r1.GetComponent<Rigidbody>().velocity);
        ...
        // Vector3 dia = new Vector3(l1.position.x-board.position.x, 0, l1.position.z - board.position.z);
        // var angle = Mathf.PI/180f * Vector3.Angle(dia, transform.forward);
        // var dist = Vector3.Distance(dia, new Vector3(0,0,0));
        // float newx, newz;
        // if (this.name =="Agent1")
        // {
        //     newx = dist * Mathf.Sin(angle);// + (board.position.x - woodyarea.transform.position.x);
        //     newz = dist * Mathf.Cos(angle);// + (board.position.z - woodyarea.transform.position.z);
        // }
        // else
        // {
        //     newx = dist * Mathf.Sin(angle-Mathf.PI);// + (board.position.x - woodyarea.transform.position.x);
        //     newz = dist * Mathf.Cos(angle);// + (board.position.z - woodyarea.transform.position.z);
        // }
        
        // var newl1 = new Vector3(newx, l1.position.y, newz);

        // Debug.Log(this.name + "LEFT angle= "+ angle + ", l1 pos= " + dia.ToString("F3") + ", l1 distance= " + dist + ", calculate l1 pos= " + newl1.ToString("F3") + ", board pos= " + (board.position-woodyarea.transform.position) + ", transform.forward= " + transform.forward);
        // // Debug.Log(woodyarea.name + ", " +ballrigid.velocity + ", " + ballrigid.angularVelocity + ", " + boardrigid.velocity + ", " + boardrigid.angularVelocity + ", " + l1.GetComponent<Rigidbody>().velocity + ", " +  r1.GetComponent<Rigidbody>().velocity);  
        // //////////////////////////// right hand
        // dia = new Vector3(r1.position.x-board.position.x, 0, r1.position.z - board.position.z);
        // angle = Mathf.PI/180f * Vector3.Angle(dia, transform.forward);
        // dist = Vector3.Distance(dia, new Vector3(0,0,0));
        ...
        // newl1 = new Vector3(newx, l1.position.y, newz);
        // Debug.Log(this.name + "RIGHT angle= "+ angle + ", r1 pos= " + dia.ToString("F3") + ", r1 distance= " + dist + ", calculate r1 pos= " + newl1.ToString("F3") + ", board pos= " + (board.position-woodyarea.transform.position) + ", transform.forward= " + transform.forward);
        // // Debug.Log(woodyarea.name + ", " +ballrigid.velocity + ", " + ballrigid.angularVelocity + ", " + boardrigid.velocity + ", " + boardrigid.angularVelocity + ", " + l1.GetComponent<Rigidbody>().velocity + ", " +  r1.GetComponent<Rigidbody>().velocity);  
    ...
    void DenseRewardTriMatch1()
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += " " + mb.Name + " " + (rew1 + rew2).ToString("F3");
    }
    ...
    void DenseRewardTriMatch4()
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += " " + mb.Name + " " + (reward * trimatch_ratio).ToString("F3");
    }
    ...
    void DenseRewardBoard4()
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += " " + mb.Name + " " + (reward*board_ratio).ToString("F3");
    }
    ...
    void DenseRewardBoard6()
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += ", " + mb.Name + " " + (reward*board_ratio).ToString("F3") + ", distval " + Mathf.Pow(cur_boarddist, 2f) * 1000f + ", angleval " + Mathf.Pow(cur_boardang, 2f)*0.01f;
    }
    ...
    void DenseRewardBoard7()
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += ", " + mb.Name + " " + (reward*board_ratio).ToString("F3") + ", distval " + Mathf.Pow(cur_boarddist, 2f) * 10f + ", angleval " + Mathf.Pow(cur_boardang, 2f)*0.01f;
    }
    ...
    void DenseRewardBoard8()
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += ", " + mb.Name + " " + (reward*board_ratio).ToString("F3") + ", distval " + r_dist * 0.8f + ", angleval " + r_ang * 0.2f;
    }
    ...
    void DenseRewardBoard9() // only use position to constraint the board
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += ", " + mb.Name + " " + (reward*board_ratio).ToString("F3") + ", distval " + r_dist;
    ...
    void DenseRewardBoard13()
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += ", " + mb.Name + " " + (reward*board_ratio).ToString("F3") + ", distval " + (r_dist * 0.8f).ToString("F3") + ", angleval " + (r_ang * 0.2f).ToString("F3");
    ...
    void DenseRewardBoard14()
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += ", " + mb.Name + " " + (reward*board_ratio).ToString("F3");// + ", distval " + r_dist * 0.8f + ", angleval " + r_ang * 0.2f;
        // log += ", " + mb.Name + " " + (reward*board_ratio).ToString("F3") + ", distval " + cur_boarddist + ", angleval " + cur_boardang;
    }
    ...
    void DenseRewardBallBalance4()
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += " " + mb.Name + " " + (reward * ball_balance_ratio).ToString("F3");
    }
    ...
    void DenseRewardBallBalance5()
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += ", " + mb.Name + ", " + (reward * ball_balance_ratio).ToString("F3") ;//+ ", " + cur_balldist.ToString("F3");
    ...
    void DenseRewardComfortPose2()
    {
        ...
        MethodBase mb = MethodBase.GetCurrentMethod();
        log += " " + mb.Name + " " + (reward*comfort_pose_ratio).ToString("F3");
    }
