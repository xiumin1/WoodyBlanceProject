...
public class WoodyArea_mtarget : MonoBehaviour
{
    ...
    public float FollowPath(float rad)
    {
        ...
        float x = path.localPosition.x + path.lossyScale.x/2f*Mathf.Cos(rad);
        float z = path.localPosition.z + path.lossyScale.z/2f*Mathf.Sin(rad);
