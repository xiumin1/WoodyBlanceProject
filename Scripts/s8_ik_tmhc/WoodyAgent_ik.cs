...
public class WoodyAgent_ik : Agent
{
    //------------------------------------------------------------------------------------------
    ...
    public WoodyAgent_ik otheragent; 
    ...
    public HandReach handreach;

    // int stepcounts = 0;

    public bool currentEpisodeConnected = false;
    ...
    void TakeAction2(ActionSegment<float> vectoraction)
    {
        for (var i = 0; i < action_count; i++)
        {
            vectoraction[i] = Mathf.Clamp(vectoraction[i], -1, 1f)*force_magnitude * Time.deltaTime;
            // Debug.Log(i.ToString() + "th action= " + vectoraction[i].ToString());
        }

        var angle = upper_arm_right.transform.localEulerAngles;
        upper_arm_right.transform.localEulerAngles = new Vector3(angle.x + vectoraction[0], angle.y + vectoraction[1], angle.z + vectoraction[2]);

        angle = lower_arm_right.transform.localEulerAngles;
        lower_arm_right.transform.localEulerAngles = new Vector3(angle.x + vectoraction[3], angle.y + vectoraction[4], angle.z + vectoraction[5]);

        angle = hand_right.transform.localEulerAngles;
        hand_right.transform.localEulerAngles = new Vector3(angle.x + vectoraction[6], angle.y + vectoraction[7], angle.z + vectoraction[8]);

        angle = upper_arm_left.transform.localEulerAngles;
        upper_arm_left.transform.localEulerAngles = new Vector3(angle.x + vectoraction[9], angle.y + vectoraction[10], angle.z + vectoraction[11]);

        angle = lower_arm_left.transform.localEulerAngles;
        lower_arm_left.transform.localEulerAngles = new Vector3(angle.x + vectoraction[12], angle.y + vectoraction[13], angle.z + vectoraction[14]);

        angle = hand_left.transform.localEulerAngles;
        hand_left.transform.localEulerAngles = new Vector3(angle.x + vectoraction[15], angle.y + vectoraction[16], angle.z + vectoraction[17]);
    ...
    void TakeAction3(ActionSegment<float> vectoraction)
    {
        ...
        lower_arm_right.AddForce(lower_arm_right_torque, ForceMode.Force); //, joints["hand_right"].transform.position);
        hand_right.AddForce(hand_right_torque, ForceMode.Force); //, limbends["hand_right_end"].position);

        upper_arm_left.AddForce(upper_arm_left_torque, ForceMode.Force); //, joints["lower_arm_right"].transform.position);
        lower_arm_left.AddForce(lower_arm_left_torque, ForceMode.Force); //, joints["hand_left"].transform.position);
        hand_left.AddForce(hand_left_torque, ForceMode.Force); //, limbends["hand_left_end"].position); 
    ...
    void TakeAction4(ActionSegment<float> vectoraction)
    {
        ...
        upper_arm_right.transform.RotateAround(upper_arm_right_joint.position, Vector3.forward, vectoraction[0]);
        upper_arm_right.transform.RotateAround(upper_arm_right_joint.position, Vector3.right, vectoraction[1]);
        upper_arm_right.transform.RotateAround(upper_arm_right_joint.position, Vector3.up, vectoraction[2]);

        lower_arm_right.transform.RotateAround(lower_arm_right_joint.position, Vector3.forward, vectoraction[3]);
        lower_arm_right.transform.RotateAround(lower_arm_right_joint.position, Vector3.right, vectoraction[4]);
        lower_arm_right.transform.RotateAround(lower_arm_right_joint.position, Vector3.up, vectoraction[5]);

        hand_right.transform.RotateAround(hand_right_joint.position, Vector3.forward, vectoraction[6]);
        hand_right.transform.RotateAround(hand_right_joint.position, Vector3.right, vectoraction[7]);
        hand_right.transform.RotateAround(hand_right_joint.position, Vector3.up, vectoraction[8]);

        upper_arm_left.transform.RotateAround(upper_arm_left_joint.position, Vector3.forward, vectoraction[9]);
        upper_arm_left.transform.RotateAround(upper_arm_left_joint.position, Vector3.right, vectoraction[10]);
        upper_arm_left.transform.RotateAround(upper_arm_left_joint.position, Vector3.up, vectoraction[11]);

        lower_arm_left.transform.RotateAround(lower_arm_left_joint.position, Vector3.forward, vectoraction[12]);
        lower_arm_left.transform.RotateAround(lower_arm_left_joint.position, Vector3.right, vectoraction[13]);
        lower_arm_left.transform.RotateAround(lower_arm_left_joint.position, Vector3.up, vectoraction[14]);

        hand_left.transform.RotateAround(hand_left_joint.position, Vector3.forward, vectoraction[15]);
        hand_left.transform.RotateAround(hand_left_joint.position, Vector3.right, vectoraction[16]);
        hand_left.transform.RotateAround(hand_left_joint.position, Vector3.up, vectoraction[17]);
    }
    ...
    void SetResetParameters()
    {
        ...
        target_maintain_time = m_ResetParams.GetWithDefault("target_maintain_time", 0.5f);
        ...
        ball_balance_ratio = m_ResetParams.GetWithDefault("ball_balance_ratio", 1f);
        board_ratio = m_ResetParams.GetWithDefault("board_ratio", 1f);
        ...
        action_index = m_ResetParams.GetWithDefault("action_index", 4f);
        ...
        board_dense_reward_index = m_ResetParams.GetWithDefault("board_dense_reward_index", 4f);
        board_sparse_reward_index = m_ResetParams.GetWithDefault("board_sparse_reward_index", 0f);

        ball_balance_dense_reward_index = m_ResetParams.GetWithDefault("ball_balance_dense_reward_index", 4f);
        ...
        time_penalty_reward = m_ResetParams.GetWithDefault("time_penalty_reward", 0f);
    }
    ...
    void DenseRewardBoard4()
    {
        cur_boarddist_y = Mathf.Abs(board.position.y - board_target_y);
        var reward = 0.0f;
        reward += Mathf.Exp(-cur_boarddist_y * 25f);
    ...
    void DenseRewardBoard5()
    {
        cur_boarddist_y = Mathf.Abs(board.position.y - board_target_y);
        var reward = 0.0f;
        reward += Mathf.Exp(-cur_boarddist_y*cur_boarddist_y*625f);
        AddReward(reward * board_ratio);
    ...
    public override void Heuristic(in ActionBuffers actionsOut)
    {
        ...
        if (Input.GetKey(KeyCode.T)) actions[5] = torqueforce;
        if (Input.GetKey(KeyCode.G)) actions[5] = -torqueforce;
        if (Input.GetKey(KeyCode.Y)) actions[6] = torqueforce;
        if (Input.GetKey(KeyCode.H)) actions[6] = -torqueforce;
        if (Input.GetKey(KeyCode.U)) actions[8] = torqueforce ;
        if (Input.GetKey(KeyCode.J)) actions[8] = -torqueforce;
        if (Input.GetKey(KeyCode.I)) actions[9] = torqueforce;
        if (Input.GetKey(KeyCode.K)) actions[9] = -torqueforce;
        if (Input.GetKey(KeyCode.O)) actions[10] = torqueforce;
        if (Input.GetKey(KeyCode.L)) actions[10] = -torqueforce;
        if (Input.GetKey(KeyCode.Z)) actions[11] = torqueforce;
        if (Input.GetKey(KeyCode.X)) actions[11] = -torqueforce;
        if (Input.GetKey(KeyCode.C)) actions[12] = torqueforce;
        if (Input.GetKey(KeyCode.V)) actions[12] = -torqueforce;
        if (Input.GetKey(KeyCode.B)) actions[14] = torqueforce;
        if (Input.GetKey(KeyCode.N)) actions[14] = -torqueforce;
        if (Input.GetKey(KeyCode.M)) actions[15] = torqueforce;
        if (Input.GetKey(KeyCode.P)) actions[15] = -torqueforce;
        if (Input.GetKey(KeyCode.LeftArrow)) actions[17] = torqueforce;
        if (Input.GetKey(KeyCode.RightArrow)) actions[17] = -torqueforce;
    ...
    {
       if (action[0] > 0) Debug.Log(this.name + " Q pressed, upper_arm_right X joint force increased" + " " + action[0] + " " + upper_arm_right.transform.localEulerAngles);
       if (action[0] < 0) Debug.Log(this.name + " A pressed, upper_arm_right X joint force decreased" + " " + action[0] + " " + upper_arm_right.transform.localEulerAngles);
       if (action[1] > 0) Debug.Log(this.name + " W pressed, upper_arm_right Y joint force increased" + " " + action[1] + " " + upper_arm_right.transform.localEulerAngles);
       if (action[1] < 0) Debug.Log(this.name + " S pressed, upper_arm_right Y joint force decreased" + " " + action[1] + " " + upper_arm_right.transform.localEulerAngles);
       if (action[2] > 0) Debug.Log(this.name + " E pressed, upper_arm_right Z joint force increased" + " " + action[2] + " " + upper_arm_right.transform.localEulerAngles);
       if (action[2] < 0) Debug.Log(this.name + " D pressed, upper_arm_right Z joint force decreased" + " " + action[2] + " " + upper_arm_right.transform.localEulerAngles);
       if (action[3] > 0) Debug.Log(this.name + " R pressed, lower_arm_right X joint force increased" + " " + action[3] + " " + lower_arm_right.transform.localEulerAngles);
       if (action[3] < 0) Debug.Log(this.name + " F pressed, lower_arm_right X joint force decreased" + " " + action[3] + " " + lower_arm_right.transform.localEulerAngles);
       if (action[5] > 0) Debug.Log(this.name + " T pressed, lower_arm_right Z joint force increased" + " " + action[5] + " " + lower_arm_right.transform.localEulerAngles);
       if (action[5] < 0) Debug.Log(this.name + " G pressed, lower_arm_right Z joint force decreased" + " " + action[5] + " " + lower_arm_right.transform.localEulerAngles);
       if (action[6] > 0) Debug.Log(this.name + " Y pressed, hand_right X joint force increased" + " " + action[6] + " " + hand_right.transform.localEulerAngles);
       if (action[6] < 0) Debug.Log(this.name + " H pressed, hand_right X joint force decreased" + " " + action[6] + " " + hand_right.transform.localEulerAngles);
       if (action[8] > 0) Debug.Log(this.name + " U pressed, hand_right Z joint force increased" + " " + action[8] + " " + hand_right.transform.localEulerAngles);
       if (action[8] < 0) Debug.Log(this.name + " J pressed, hand_right Z joint force decreased" + " " + action[8] + " " + hand_right.transform.localEulerAngles);
       if (action[9] > 0) Debug.Log(this.name + " I pressed, upper_arm_left X joint force increased" + " " + action[9] + " " + upper_arm_left.transform.localEulerAngles);
       if (action[9] < 0) Debug.Log(this.name + " K pressed, upper_arm_left X joint force decreased" + " " + action[9] + " " + upper_arm_left.transform.localEulerAngles);
       if (action[10] > 0) Debug.Log(this.name + " O pressed, upper_arm_left Y joint force increased" + " " + action[10] + " " + upper_arm_left.transform.localEulerAngles);
       if (action[10] < 0) Debug.Log(this.name + " L pressed, upper_arm_left Y joint force decreased" + " " + action[10] + " " + upper_arm_left.transform.localEulerAngles);
       if (action[11] > 0) Debug.Log(this.name + " Z pressed, upper_arm_left Z joint force increased" + " " + action[11] + " " + upper_arm_left.transform.localEulerAngles);
       if (action[11] < 0) Debug.Log(this.name + " X pressed, upper_arm_left Z joint force decreased" + " " + action[11] + " " + upper_arm_left.transform.localEulerAngles);
       if (action[12] > 0) Debug.Log(this.name + " C pressed, lower_arm_left X joint force increased" + " " + action[12] + " " + lower_arm_left.transform.localEulerAngles);
       if (action[12] < 0) Debug.Log(this.name + " V pressed, lower_arm_left X joint force decreased" + " " + action[12] + " " + lower_arm_left.transform.localEulerAngles);
       if (action[14] > 0) Debug.Log(this.name + " B pressed, lower_arm_left Z joint force increased" + " " + action[14] + " " + lower_arm_left.transform.localEulerAngles);
       if (action[14] < 0) Debug.Log(this.name + " N pressed, lower_arm_left Z joint force decreased" + " " + action[14] + " " + lower_arm_left.transform.localEulerAngles);
       if (action[15] > 0) Debug.Log(this.name + " M pressed, hand_left X joint force increased" + " " + action[15] + " " + hand_left.transform.localEulerAngles);
       if (action[15] < 0) Debug.Log(this.name + " P pressed, hand_left X joint force decreased" + " " + action[15] + " " + hand_left.transform.localEulerAngles);
       if (action[17] > 0) Debug.Log(this.name + " leftarrow pressed, hand_left Z joint force increased" + " " + action[17] + " " + hand_left.transform.localEulerAngles);
       if (action[17] < 0) Debug.Log(this.name + " rightarrow pressed, hand_left Z joint force decreased" + " " + action[17] + " " + hand_left.transform.localEulerAngles);
    }
}
